...
public:
    static ProtoCodec& GetInstance() {
        static ProtoCodec instance;
        return instance;
    }
    ...
    // 编码消息
    bool EncodeMessage(net::Packet* packet, const protobuf::Message& message) {
        ...
        size_t i = packet->size();
        packet->resize(packet->size() + sizeof(msg_id_net) + sizeof(sub_msg_id_net) + message.ByteSize());
    ...
    std::optional<DecodeRes> DecodeMessage(net::PacketSpan span) {
        DecodeRes res = { 0 };

        uint16_t msg_id_net, sub_msg_id_net;
        if (span.size() < sizeof(msg_id_net) + sizeof(sub_msg_id_net)) return std::nullopt;

        size_t i = 0;
        std::memcpy(&msg_id_net, span.data() + i, sizeof(msg_id_net));
        i += sizeof(msg_id_net);
        std::memcpy(&sub_msg_id_net, span.data() + i, sizeof(sub_msg_id_net));
        i += sizeof(sub_msg_id_net);
        ...
        auto success = res.proto_msg->ParseFromArray(span.data() + i, span.size() - i);
        if (!success) {
            return std::nullopt;
        }
        return res;
...
#define MILLION_PROTO_MSG_DISPATCH(NAMESPACE_, PROTO_MSG_TYPE_) \
    using _MILLION_PROTO_MSG_TYPE_ = PROTO_MSG_TYPE_; \
    MILLION_MSG_HANDLE(PROTO_MSG_TYPE_, msg) { \
        auto res = ::million::ProtoCodec::GetInstance().GetMsgId(*msg->proto_msg); \
        if (!res) { \
            co_return; \
        } \
        auto&& [msg_id, sub_msg_id] = *res; \
        auto iter = _MILLION_PROTO_MSG_HANDLE_MAP_.find(::million::ProtoCodec::CalcKey(msg_id, sub_msg_id)); \
        if (iter != _MILLION_PROTO_MSG_HANDLE_MAP_.end()) { \
            co_await (this->*iter->second)(msg->session, std::move(msg->proto_msg)); \
        } \
        co_return; \
    } \
    NAMESPACE_::##NAMESPACE_##MsgId _MILLION_PROTO_MSG_HANDLE_CURRENT_MSG_ID_; \
    ::std::unordered_map<uint32_t, ::million::Task<>(_MILLION_SERVICE_TYPE_::*)(const decltype(_MILLION_PROTO_MSG_TYPE_::session)&, ::million::ProtoMsgUnique)> _MILLION_PROTO_MSG_HANDLE_MAP_ \
...
#define MILLION_PROTO_MSG_HANDLE(NAMESPACE_, SUB_MSG_ID_, MSG_TYPE_, MSG_PTR_NAME_) \
    ::million::Task<> _MILLION_PROTO_MSG_HANDLE_##MSG_TYPE_##_I(const decltype(_MILLION_PROTO_MSG_TYPE_::session)& session, ::million::ProtoMsgUnique MILLION_PROTO_MSG_) { \
    ...
    ::million::Task<> _MILLION_PROTO_MSG_HANDLE_##MSG_TYPE_##_II(const decltype(_MILLION_PROTO_MSG_TYPE_::session)& session, ::std::unique_ptr<NAMESPACE_::MSG_TYPE_> MSG_PTR_NAME_)
