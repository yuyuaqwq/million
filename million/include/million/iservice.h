...
public:
    ...
    SessionAwaiter<RecvMsgT> Call(const ServiceHandle& target, Args&&... args) {
        auto session_id = Send(target, make_msg<SendMsgT>(std::forward<Args>(args)...));
        return Recv<RecvMsgT>(session_id);
    }
    template <typename MsgT, typename ...Args>
    SessionAwaiter<MsgT> Call(const ServiceHandle& target, Args&&... args) {
        auto session_id = Send(target, make_msg<MsgT>(std::forward<Args>(args)...));
    ...
    virtual Task<> OnMsg(SessionId session_id, MsgUnique msg) = 0;
    virtual void OnStop() { }
    //virtual void OnExit() { }
...
#define MILLION_MSG_HANDLE(MSG_TYPE_, MSG_PTR_NAME_) \
    ::million::Task<> _MILLION_MSG_HANDLE_##MSG_TYPE_##_I(::million::SessionId session_id, ::million::MsgUnique MILLION_MSG_) { \
        auto msg = ::std::unique_ptr<MSG_TYPE_>(static_cast<MSG_TYPE_*>(MILLION_MSG_.release())); \
        co_await _MILLION_MSG_HANDLE_##MSG_TYPE_##_II(session_id, std::move(msg)); \
        co_return; \
    } \
    ...
    ::million::Task<> _MILLION_MSG_HANDLE_##MSG_TYPE_##_II(::million::SessionId session_id, ::std::unique_ptr<MSG_TYPE_> MSG_PTR_NAME_)
