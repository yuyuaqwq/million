...
// 比如交易场景
// 拍卖行服务开启事务，如果玩家都在线，向A发起扣除物品，向B发起添加物品 即可完成
// 如果B离线，那么该行可能不被目标节点占用，就可以直接读出来然后进行修改
// 但是也要考虑B在完成交易前登录并读取了数据，B的数据就是旧的，下次tick回写就会失败，这段时间内修改的数据全部会回滚
// 这个如何解决？
// 1. 登录时立即修改一些最后登录时间之类的，提交到sql(触发sql版本更新)，提交失败就重新读取，重新走这个登录流程
// 2. 考虑引入redis
